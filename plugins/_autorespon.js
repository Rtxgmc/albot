    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? 'Tirtabot tidak aktif' : banned ? 'kamu dibanned' : 'Fernazer Disini Knpa Tag Saya Kak?',
                '© Tirtabot',
...
https://github.com/????????
`.trim(), '© Tirtabot', 'PEMILIK BOT', '.owner', { contextInfo: { mentionedJid: [global.owner[0] + '@s.whatsapp.net'] } })
    }
